...
export const job = pgTable('job', {
  ...
  /** Skills/technologies required for this role — used for candidate match scoring */
...
export const candidate = pgTable('candidate', {
  ...
  /** Self-reported + resume-extracted skills — used for candidate match scoring */
  skills: text('skills').array().notNull().default(sql`'{}'::text[]`),
...
/**
 * In-app notifications for the candidate self-service portal.
 * Generated by the backend (match found, application status change,
 * interview scheduled, etc.) and surfaced in the candidate's notification feed.
 */
export const candidateNotification = pgTable('candidate_notification', {
  id: text('id').primaryKey().$defaultFn(() => crypto.randomUUID()),
  candidateId: text('candidate_id').notNull().references(() => candidate.id, { onDelete: 'cascade' }),
  category: candidateNotificationCategoryEnum('category').notNull(),
  title: text('title').notNull(),
  description: text('description'),
  ...
  index('candidate_notification_candidate_id_idx').on(t.candidateId),
  index('candidate_notification_candidate_read_idx').on(t.candidateId, t.isRead),
]))

/**
 * Candidate job-search preferences — drives recommended-job matching and
 * notification settings on the self-service portal. One row per candidate.
 */
export const candidatePreference = pgTable('candidate_preference', {
  candidateId: text('candidate_id').primaryKey().references(() => candidate.id, { onDelete: 'cascade' }),
  ...
  notifyInterviews: boolean('notify_interviews').notNull().default(true),
  updatedAt: timestamp('updated_at').notNull().defaultNow(),
})

/**
 * Computed match between a candidate and an open job, surfaced on the
 * candidate portal's "Matches" screen. Candidates can accept (creates an
 * application) or reject a match.
 */
export const candidateMatch = pgTable('candidate_match', {
  ...
  /** Array of { label, value } — one entry per scoring criterion */
  criteria: jsonb('criteria').$type<{ label: string; value: number }[]>().notNull(),
  /** Short human-readable strings explaining the match */
  reasons: jsonb('reasons').$type<string[]>().notNull(),
  /** Single string naming the weakest criterion, when any scores < 70 */
  gap: text('gap'),
  status: candidateMatchStatusEnum('status').notNull().default('new'),
  matchedAt: timestamp('matched_at').notNull().defaultNow(),
  updatedAt: timestamp('updated_at').notNull().defaultNow(),
}, (t) => ([
  index('candidate_match_candidate_id_idx').on(t.candidateId),
  index('candidate_match_job_id_idx').on(t.jobId),
  uniqueIndex('candidate_match_candidate_job_idx').on(t.candidateId, t.jobId),
]))
...
export const candidateRelations = relations(candidate, ({ one, many }) => ({
  ...
  notifications: many(candidateNotification),
  preference: one(candidatePreference, { fields: [candidate.id], references: [candidatePreference.candidateId] }),
  matches: many(candidateMatch),
}))

export const candidateNotificationRelations = relations(candidateNotification, ({ one }) => ({
  candidate: one(candidate, { fields: [candidateNotification.candidateId], references: [candidate.id] }),
}))

export const candidatePreferenceRelations = relations(candidatePreference, ({ one }) => ({
  candidate: one(candidate, { fields: [candidatePreference.candidateId], references: [candidate.id] }),
}))

export const candidateMatchRelations = relations(candidateMatch, ({ one }) => ({
  candidate: one(candidate, { fields: [candidateMatch.candidateId], references: [candidate.id] }),
  job: one(job, { fields: [candidateMatch.jobId], references: [job.id] }),
}))
